...
for file in sorted(os.listdir(path)):
    if file.endswith('.sgf'):
        ...
        for i in range(len(moves) - 1):
            ...
            move = moves[i]
            color = move[0]
            coord_x = letter_to_num(move[2])
            coord_y = letter_to_num(move[3])
            y = coord_to_pos(coord_x, coord_y)
            ...
            # for p in range(x.n):
            #     for q in range(x.n):
            #         v = x.planes[TURNS_SINCE_8].get(p, q) + x.planes[TURNS_SINCE_7].get(p, q)
            #         x.planes[TURNS_SINCE_8].set(p, q, v)
        ...
        with open('../converted_database/' + new_file, 'w') as f:
            # TODO: iterate through all states, not just the first one
            write_header(f)
